...
def solar_health_check():
    ...
    if len(daytime) > 0:
        ...
        if pr < 0.75:
            ...
            alerts.append({
                ...
                "severity": "INFO",
                "message" : (
                    f"Performance Ratio {pr:.2f} — slight degradation. "
                    f"Monitor over next 3 days."
                ),
                "action"  : "Monitor"
            })
    ...
    if len(df) >= 4:
        ...
        if recent_drop > 50 and 9 <= hour_now <= 16:
            alerts.append({
                ...
                "severity": "CRITICAL",
                "message" : (
                    f"Solar output dropped {recent_drop:.0f} kW in last hour "
    ...
    try:
        weather_r = requests.get(
            ...
            params={
                "latitude"     : 22.57,   # Kolkata — EDIT per customer
                "longitude"    : 88.36,
        ...
        if weather_r.status_code == 200:
            ...
            if max_wind > 15 and max_rain > 70:
                alerts.append({
                    ...
                    "severity": "WARNING",
                    "message" : (
                        f"Storm forecast in next 48 hours. "
        ...
        if last_week > 5:
            ...
            if week_drop > 15:
                alerts.append({
                    ...
                    "severity": "WARNING",
                    "message" : (
                        f"Solar output down {week_drop:.0f}% vs last week "
    ...
    return {
        ...
        "alerts"       : alerts,
        "performance_ratio": round(pr if 'pr' in dir() else 1.0, 3),

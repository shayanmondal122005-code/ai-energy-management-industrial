...
def solar_health_check():
    ...
    """
    if len(readings) < 96:  # need at least 1 day of data
    ...
    alerts = []
    pr = 1.0  # Default performance ratio
    ...
    if len(daytime) > 0:
        ...
        if pr < 0.75:
            alerts.append({
                "type": "SOILING", 
                "severity": "WARNING", 
                "message": f"Panel Performance Ratio {pr:.2f} — below 0.75 threshold. Panels likely dirty.",
            ...
            alerts.append({
                "type": "SOILING", 
                "severity": "INFO", 
                "message": f"Performance Ratio {pr:.2f} — slight degradation noticed.",
    ...
    if len(df) >= 4:
        ...
        if recent_drop > 50 and 9 <= hour_now <= 16:
            alerts.append({
                "type": "SUDDEN_DROP", 
                "severity": "CRITICAL", 
                "message": f"Solar output dropped {recent_drop:.0f} kW rapidly during daylight hours without weather variance.",
        ...
        if weather_r.status_code == 200:
            ...
            if max_wind > 15 and max_rain > 70:
                alerts.append({
                    "type": "STORM", 
                    "severity": "WARNING", 
                    "message": f"Storm forecast incoming. Max wind: {max_wind:.0f} km/h, Rain probability: {max_rain:.0f}%.",

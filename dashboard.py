...
def forecast_24h(model, df_raw, err_hr, solar_cap=200):
...
def brain(fc_df, soc_trace, cur_soc, cur_load, cur_solar, cur_h, tariff):
    alerts, tags = [], []
    ...
    hrs_left = (cur_soc-10)*500/max(1,abs(net_now)*100/95) if net_now<0 else 999
    ...
    if is_cheap and cur_soc < 75 and not is_peak:
        saved = (75-cur_soc)/100*500*(tariff["peak"]-tariff["cheap"])
        alerts.append(("INFO",
            f"🔵 Cheap tariff active — ₹{tariff['cheap']}/kWh. "
            f"Charging now saves ₹{saved:.0f} vs peak import.",
            "CHARGE NOW"))
        tags.append("dt-blue")
    ...
    if peak_kw > 400:
        ...
        alerts.append(("WARNING",
            ...
            f"Battery shaving will save ₹{dc_save:.0f} in demand charges.",
            "DEMAND SHAVE"))
        tags.append("dt-amber")
    ...
    if is_peak and cur_soc > 30 and net_now < 0:
        ...
        alerts.append(("INFO",
            f"🔵 Peak rate ₹{tariff['peak']}/kWh. "
            f"Battery covering {abs(net_now):.0f} kW — saving ₹{hr_save:.0f}/hr.",
            "DISCHARGING"))
        tags.append("dt-amber")
...
def solar_health_local(df, solar_cap=200):
    ...
    alerts = []
    day = df[df.index.hour.between(9,15)]
...
if src == "🔵 Live (Render backend)":
    with st.spinner("Connecting to live backend..."):
        df_backend, tag = fetch_backend(hours=600)
    if df_backend is not None and len(df_backend) >= 48:
        ...
        st.sidebar.warning("Backend has < 48 readings. Using demo data.\nRun feeder.py to populate.")
    ...
    if df_raw is None or len(df_raw) < 168:
        st.warning("Need 1 week minimum. Using demo data.")
        df_raw = synthetic_data(n_days=365, solar_kw=solar_cap)
    else:
        data_tag = "csv"
    live_reading  = None
...
with st.spinner("Generating 24-hour forecast..."):
    fc_df, soc_trace = forecast_24h(model, df_raw, err_hr, solar_cap)
...
for col, val, lbl in [
    (s1, f"₹{savings['daily']:,.0f}",             "Daily saving"),
    (s2, f"₹{savings['monthly']/1000:.1f}K",      "Monthly saving"),
    (s3, f"₹{savings['annual']/100000:.1f}L",     "Annual saving"),
...
st.markdown(
    ...
    f'{tariff_name} · ₹{tariff["cheap"]}/kWh cheap · '
    f'₹{tariff["normal"]}/kWh normal · ₹{tariff["peak"]}/kWh peak · '
    f'₹{tariff["demand_per_kw"]}/kW demand charge · '
    f'Software fee: ₹40,000/month</div>',
    unsafe_allow_html=True
)

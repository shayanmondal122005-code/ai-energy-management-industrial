...
BACKEND_URL = "https://ai-energy-managementat12.onrender.com/docs"
...
with st.sidebar:
    ...
    # ── Backend simulate button ──
    st.markdown(
        '<div style="font-size:10px;letter-spacing:2px;'
        'text-transform:uppercase;color:#1d4ed8;'
        'font-weight:600;margin-bottom:8px">Backend</div>',
        unsafe_allow_html=True
    )
...
if data_mode == "Render backend":
    with st.spinner("Fetching data from Render backend..."):
        df_raw, backend_status = fetch_backend_data(hours=360)
    if backend_status == "live":
        data_tag = "live"
        st.success(
            f"✓ Backend data loaded — "
            f"{len(df_raw)} hours "
        ...
        st.warning(
            "Backend unreachable — using synthetic data. "
            f"Set BACKEND_URL in dashboard.py to connect."
        )

elif data_mode == "Upload CSV" and uploaded_file is not None:
...
# ════════════════════════════════════════════════════════
# ── SOLAR HEALTH ALERTS (from backend) ──
# ════════════════════════════════════════════════════════
...
if health and health.get('alerts'):
    ...
    for alert in health['alerts']:
        ...
        st.markdown(
            ...
            f'{alert["message"]}<br>'
            f'<small style="opacity:0.7">Action: {alert["action"]}</small>'
            f'</div>',
            unsafe_allow_html=True
        )
...
fig1.update_layout(title=dict(text='', x=0))
